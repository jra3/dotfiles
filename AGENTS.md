...
**Codex skill:** Use `/omarchy` for help with Hyprland config, keybindings, monitors, themes, input devices, or any `~/.config/hypr/` files. Omarchy 4 also ships its own skill at `$OMARCHY_PATH/default/agents/skills/omarchy/`, which tracks the current release — prefer it when the two disagree.
...
Omarchy already owns that directory: `install/config/theme-system.sh` creates it
and `chmod a+rw`s it so `omarchy theme set` can write `color.json` unprivileged.
`configure-system` adds `extensions.json`, which force-installs **floccus**
(`fnaicdffflnofjppbagibeoednhnbjhg`) into every Helium profile on the machine.
...
wire. Verified 2026-08-31: floccus 5.10.3 installs into a fresh profile.
...
refresh it. Note that policy installs the extension but not its sync account —
that stays per-machine setup in the floccus options page.
...
| AI | **Codex** | AI-powered coding assistant |
...
- `ghostty/` - Ghostty terminal emulator, no longer the default: `xdg-terminals.list`
  moved to `kitty/` on 2026-09-08 (see the `kitty/` entry).
...
- `ccstatusline/` - Codex status line: the `ccstatusline` layout plus the `cc-pr-widget` PR/CI segment it shells out to; see `ccstatusline/README.md`
